...
fields <- nih_fields %>%
  filter(response_name %in% #specify the headers you want
    c("appl_id", "terms", "activity_code", "project_title", "fiscal_year", "award_amount", "is_active", "project_start_date", "abstract_text","program_officers", "activity_type", #"org_names",
  "organization_type")) %>% pull(include_name)

req <- make_req(criteria = list(fiscal_years = 2021,
